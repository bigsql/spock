								   ...
								   List *replication_sets)
{
	...
	if (spock_remote_function_exists(conn, "spock", "show_repset_table_info_by_target", 3, NULL))
	{
		...
		appendStringInfo(&query,
						 "SELECT i.relid, i.nspname, i.relname, i.att_list,"
						 "       i.has_row_filter, i.nsptarget, i.reltarget"
						 "  FROM spock.show_repset_table_info_by_target(%s, %s, ARRAY[%s]) i",
						 PQescapeLiteral(conn, rv->schemaname, strlen(rv->schemaname)),
						 PQescapeLiteral(conn, rv->relname, strlen(rv->relname)),
						 repsetarr.data);
	}
...
pg_logical_get_remote_repset_sequences(PGconn *conn, List *replication_sets)
{
	...
	initStringInfo(&query);
	// TODO limit to spock >= 2.3 ?
	appendStringInfo(&query,
			 "select c.relnamespace::regnamespace as set_nspname, c.relname as set_seqname FROM spock.replication_set_seq r "
